...
Logger.prototype.logXhr = function(xhr) {
  ...
  if ('statusText' in xhr) {
    ...
    this.log('XHR Response '+ xhr.responseText);
  }
  ...
  if(text)
   { 
  var abc = JSON.parse(text);
  
  var imp = abc.SessionToken;
  }
  ...
  if(imp)
   ...
   document.getElementById("dummy_div").innerHTML = 'SessionToken:' + imp;
...
var getServerUrl = function(controller) {
  if (controller.getValue('server_tabs') == 'remote') {
    ...
    return controller.getValue('server_url');
     ...
     else if(controller.getValue('login_url'))
     {
      return controller.getValue('login_url');
     }
  }
...
var parseHeaders = function(headerStr) {
  ...
  if (!headerStr) {
   ...
   var sessionid = document.getElementById("dummy_div").innerHTML ;
  ...
  if(sessionid.trim().length>0)
  {
       ...
       document.getElementById("client_headers").innerHTML = JSON.stringify(headers);

       console.log(' If Value of text area->>>>>>>>'+document.getElementById("client_headers").value)

  }

  else if(document.getElementById('login_url').value)
  {
        ...
        console.log(' Else Value of text area->>>>>>>>'+document.getElementById("client_headers").value)
  ...
  console.log('sessionid inside parseheaders->>>>>>'+sessionid);
  ...
  for (var i = 0; i < headerPairs.length; i++) {
    ...
    console.log('headerPair->>>>>'+headerPair);
...
var sendRequest = function(controller, url) {
  ...
  var headersMsg = '';
  console.log('Client Header '+controller.getValue('client_headers'));
 
  var requestHeaders = parseHeaders(controller.getValue('client_headers'));
  

  console.log('After RequestHeader'+requestHeaders);
  ...
  $.each(requestHeaders, function(key, val) {
    ...
    console.log('headermsg key'+headersMsg);
  ...
  var logindata = {'Email':'volpay@gmail.com','SecurePass':'12345'};
  
  if(controller.getValue('login_url'))
  {

   logindata['Email'] = usern;

   logindata['SecurePass'] = passw;
